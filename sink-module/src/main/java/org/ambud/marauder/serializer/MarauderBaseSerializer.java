...
//		Iterator<Entry<String, String>> itr = kvp.iterator();
		rowKey = getRowKey(constructDefaultRowKey(event.getHeaders(), 0), 
				event.getHeaders());		
...
//			}
//		}
		putReq.add(MarauderParserConstants.MARAUDER_CF_HEADERS, "v".getBytes(), event.getHeaders().toString().getBytes());
//		if(event.getBody()!=null){
//			putReq.add(MarauderParserConstants.MARAUDER_CF_MESSAGE, MarauderParserConstants.MARAUDER_KEY_MESSAGE, compressEventBody(event.getBody()));
//		}
...
//		base[0] = (byte) random.nextInt(10);
		...
		System.arraycopy(
				time, 
				2, base, 1, time.length-2);
		System.arraycopy(temp, 0, base, 9, temp.length);
//		System.out.println(Arrays.toString(time)+" "+timeValue+" "+ByteBufferUtil.toLong(ByteBuffer.wrap(time)));
		return base;
	}
