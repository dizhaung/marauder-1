...
public class NumericHexIntEventCondition extends BaseEventCondition {
	
	private ConditionOperator operator;
	private int condition;

	public NumericHexIntEventCondition(MarauderEventTypes type, ConditionOperator operator, String attribute, int condition) {
	...
	public boolean evaluate(MarauderBaseEvent event) {
		if(super.evaluate(event)) {
			int value = 0;
			try{
				value = Integer.parseInt(event.getHeaders().get(attribute), 16);
			}catch(Exception e) {
				return false;
			}
			switch(operator) {
