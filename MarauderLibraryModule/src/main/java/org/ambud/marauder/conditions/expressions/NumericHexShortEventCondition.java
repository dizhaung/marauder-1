...
public class NumericHexShortEventCondition extends BaseEventCondition {
	
	private ConditionOperator operator;
	private int condition;

	public NumericHexShortEventCondition(MarauderEventTypes type, ConditionOperator operator, String attribute, short condition) {
	...
	public boolean evaluate(MarauderBaseEvent event) {
		if(super.evaluate(event)) {
			int value = 0;
			try{
				value = Short.parseShort(event.getHeaders().get(attribute), 16);
			}catch(Exception e) {
				return false;
			}
			switch(operator) {
